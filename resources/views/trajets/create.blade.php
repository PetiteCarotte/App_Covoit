<!-- Page Publication d'un covoiturage -->
 
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 light:text-gray-200 leading-tight">
            {{ __('Publication d\'un covoiturage') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white light:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h1 class="text-2xl mb-4">Publier un nouveau covoiturage</h1>
                ...
                <form action="{{ route('trajets.store') }}" method="POST">
                    ...
                    <div class="flex justify-between mb-4">
                        ...
                        <div>
                            <button type="button" id="swap-locations" class="btn btn-outline-secondary mt-2 mb-4 mt-4"><i class="bi bi-arrow-left-right"></i></button>
                        </div>
                        ...
                        <div class="w-1/2 pl-2">
                            ...
                            <datalist id="basesMilitaires">
                                @foreach($basesMilitaires as $baseMilitaire)
                                    <option
                                        value="{{ $baseMilitaire->nom_de_la_base }}"
                    ...
                    <div class="flex justify-between mb-4">
                        <div class="w-1/2 pr-2">
                            <x-input-label for="qte_bagages" :value="__('Quantité de bagages (en L)')" />
                            <x-text-input id="qte_bagages" class="form-control mt-1 w-full" type="number"
                                name="qte_bagages" min="0" value="{{ old('qte_bagages') }}" required />
                            <x-input-error :messages="$errors->get('qte_bagages')" class="mt-2" />
                        </div>

                        <div class="w-1/2 pl-2">
                            <x-input-label for="nbr_places" :value="__('Nombre de places passagers')" />
                            <x-text-input id="nbr_places" class="form-control mt-1 w-full" type="number"
                                name="nbr_places" min="1" value="{{ old('nbr_places') }}" required />
                            <x-input-error :messages="$errors->get('nbr_places')" class="mt-2" />
                        </div>
                    </div>
