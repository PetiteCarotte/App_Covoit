<nav x-data="{ open: false }" class="bg-white light:bg-gray-800 border-b border-gray-100 light:border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                <!-- Logo -->
                <div class="shrink-0 flex items-center cursor-pointer" onclick="window.location.href='{{ route('dashboard') }}';">
                    <x-application-logo class="block h-9 w-auto fill-current text-gray-800 light:text-gray-200" />
                </div>

                <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard') || request()->routeIs('dashboard.search')">
                        {{ __('Recherche') }}
                    </x-nav-link>
                </div>
                ...
                <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('trajets.create')" :active="request()->routeIs('trajets.create')">
                        {{ __('Publication') }}
                    </x-nav-link> 
                </div>
                ...
                <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('trajets.vosTrajets')" :active="request()->routeIs('trajets.vosTrajets')">
                        {{ __('Vos trajets') }}
                    </x-nav-link>
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard') || request()->routeIs('trajets.search')">
                {{ __('Recherche') }}
            </x-responsive-nav-link>
        </div>

        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('trajets.create')" :active="request()->routeIs('trajets.create')">
                {{ __('Publication') }}
            </x-responsive-nav-link>
        </div>

        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('trajets.vosTrajets')" :active="request()->routeIs('trajets.vosTrajets')">
                {{ __('Vos trajets') }}
            </x-responsive-nav-link>
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 light:border-gray-600">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800 light:text-gray-200">{{ Auth::user()->firstname }} {{ Auth::user()->name }}</div>
